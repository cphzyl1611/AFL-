#!/usr/bin/env python3
"""SE-fAnoGAN-ES-style online filter prototype for Alfresco metadata_update.

The wrapper is used by a representative AFL++ smoke over application/json
metadata semantics. It performs local rule filtering, AE v1 scoring, optional
fAnoGAN candidate scoring, and then forwards accepted inputs to the metadata
mock classifier.
"""
...
from targets.alfresco_metadata_update_mock import classify_metadata  # noqa: E402
...
def parse_metadata_payload(data: bytes) -> dict[str, Any]:
    payload = json.loads(data.decode("utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("metadata_update input requires JSON object payload")
    return payload
...
def score_ae(payload: dict[str, Any]) -> dict[str, Any]:
    return get_ae_scorer().score_metadata_payload(payload)


def score_fanogan(payload: dict[str, Any]) -> dict[str, Any]:
    return get_fanogan_scorer().score_metadata_payload(payload)


def evaluate(data: bytes, mode: str, fanogan_enabled: bool) -> dict[str, Any]:
    start = time.perf_counter()
    rule_pass, rule_reason = classify_metadata(data)
    ...
    if not rule_pass:
        ...
        final_pass = True
        try:
            payload = parse_metadata_payload(data)
        except Exception as exc:  # noqa: BLE001 - rule/parser mismatch is a controlled reject.
            final_pass = False
            reject_reason = f"rule_parse_error:{exc}"

    if final_pass and payload is not None and mode in {"rule_ae", "rule_ae_fanogan"}:
        try:
            ae_result = score_ae(payload)
    ...
    if final_pass and payload is not None and fanogan_should_run:
        try:
            fanogan_result = score_fanogan(payload)
    ...
    if final_pass:
        sent_to_target = True
        target_valid, target_reason = classify_metadata(data)
        if not target_valid:
