#!/usr/bin/env python3
"""Summarize Alfresco multipart_upload AFL++ online filter smoke evidence."""
...
OUT_DIR = Path(os.environ.get("OUT_DIR", ROOT / "out" / "alfresco_afl_multipart_online_filter_smoke_latest"))
...
METRIC_SEMANTICS = (
    "Representative AFL++ mutation-chain smoke with SE-fAnoGAN-ES-style online filter prototype over "
    "Alfresco multipart_upload mock semantics; mock target only, not full SE-fAnoGAN-ES and not real "
    "Alfresco service."
)
HEADER = [
    "mode",
...
def main() -> int:
    ...
    nv_err_exec = int_stat(stats, "nv_err_exec", saved_crashes + saved_hangs)
    ...
    filename_detected_count = sum(1 for item in records if item.get("filename_detected"))
    content_type_detected_count = sum(1 for item in records if item.get("content_type_detected"))
    ...
    row = [
        "afl_multipart_online_filter_smoke",
        nv_total_valid_exec,
        nv_err_exec,
        f"{nv_err_rate:.6f}",
        saved_hangs,
    ...
    report = {
        ...
        "online_filter": {
            ...
            "filename_detected_count": filename_detected_count,
            "content_type_detected_count": content_type_detected_count,
        },
        "boundary": [
            "multipart_upload online filter smoke only",
            ...
            "fAnoGAN candidate has not replaced AE v1",
        ],
    }
