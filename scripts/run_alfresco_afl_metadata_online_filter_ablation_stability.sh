...
STABILITY_OUT_DIR="${STABILITY_OUT_DIR:-$ROOT/out/alfresco_afl_metadata_online_filter_ablation_stability}"
RUNS="${RUNS:-3}"
DUR="${DUR:-20}"
...
if [[ -e "$STABILITY_OUT_DIR" ]]; then
  echo "[ERR] metadata stability output directory already exists: $STABILITY_OUT_DIR" >&2
...
run_mode_once() {
  ...
  echo "[*] Metadata online filter ablation stability mode=$mode run=$run_id fanogan_enabled=$fanogan_enabled"
  OUT_DIR="$run_dir" \
    ...
    bash "$ROOT/scripts/run_alfresco_afl_metadata_online_filter_smoke.sh"

  OUT_DIR="$run_dir" "$PYTHON_BIN" "$ROOT/scripts/summarize_alfresco_afl_metadata_online_filter_smoke.py"
}
...
echo "[OK] Metadata online filter ablation stability runs completed under $STABILITY_OUT_DIR"
