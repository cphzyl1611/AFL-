...
IN_DIR="${IN_DIR:-$ROOT/in/alfresco_afl_multipart_upload_smoke}"
OUT_DIR="${OUT_DIR:-$ROOT/out/alfresco_afl_multipart_online_filter_smoke_latest}"
TARGET="${TARGET:-$ROOT/targets/alfresco_multipart_upload_online_filter_wrapper.py}"
...
if [[ ! -f "$TARGET" ]]; then
  echo "[ERR] multipart online filter wrapper missing: $TARGET" >&2
  exit 2
fi
if [[ -e "$OUT_DIR" ]]; then
...
echo "[*] Running Alfresco multipart_upload AFL++ online filter smoke"
echo "[*] input=$IN_DIR"
echo "[*] output=$OUT_DIR"
echo "[*] duration=${DUR}s"
...
echo "[OK] AFL++ multipart_upload online filter smoke finished with status=$status"
