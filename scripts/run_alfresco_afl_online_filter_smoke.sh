...
OUT_DIR="${OUT_DIR:-$ROOT/out/alfresco_afl_online_filter_smoke_latest}"
TARGET="${TARGET:-$ROOT/targets/alfresco_content_update_online_filter_wrapper.py}"
...
if [[ ! -f "$TARGET" ]]; then
  echo "[ERR] online filter wrapper missing: $TARGET" >&2
  exit 2
fi
if [[ -e "$OUT_DIR" ]]; then
...
echo "[*] Running representative AFL++ online filter smoke"
echo "[*] input=$IN_DIR"
echo "[*] output=$OUT_DIR"
echo "[*] duration=${DUR}s"
...
echo "[OK] AFL++ online filter smoke finished with status=$status"
