#!/usr/bin/env python3
"""Summarize Alfresco metadata_update AFL++ online filter smoke evidence."""
...
OUT_DIR = Path(os.environ.get("OUT_DIR", ROOT / "out" / "alfresco_afl_metadata_online_filter_smoke_latest"))
...
METRIC_SEMANTICS = (
    ...
    "Alfresco metadata_update mock semantics; mock target only, not full SE-fAnoGAN-ES and not real "
    "Alfresco service."
)
HEADER = [
    "mode",
...
def main() -> int:
    ...
    row = [
        "afl_metadata_online_filter_smoke",
        nv_total_valid_exec,
        nv_err_exec,
        f"{nv_err_rate:.6f}",
        saved_hangs,
    ...
    report = {
        ...
        "boundary": [
            "metadata_update online filter smoke only",
