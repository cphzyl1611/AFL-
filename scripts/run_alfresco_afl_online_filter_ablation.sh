...
ABLATION_OUT_DIR="${ABLATION_OUT_DIR:-$ROOT/out/alfresco_afl_online_filter_ablation}"
DUR="${DUR:-20}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

if [[ -e "$ABLATION_OUT_DIR" ]]; then
  echo "[ERR] ablation output directory already exists: $ABLATION_OUT_DIR" >&2
...
run_mode() {
  ...
  echo "[*] Online filter ablation mode=$mode fanogan_enabled=$fanogan_enabled"
  OUT_DIR="$mode_out_dir" \
    ...
    bash "$ROOT/scripts/run_alfresco_afl_online_filter_smoke.sh"

  OUT_DIR="$mode_out_dir" "$PYTHON_BIN" "$ROOT/scripts/summarize_alfresco_afl_online_filter_smoke.py"
}
...
echo "[OK] Online filter ablation runs completed under $ABLATION_OUT_DIR"
