...
ABLATION_OUT_DIR="${ABLATION_OUT_DIR:-$ROOT/out/alfresco_afl_metadata_online_filter_ablation}"
DUR="${DUR:-20}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

if [[ -e "$ABLATION_OUT_DIR" ]]; then
  echo "[ERR] metadata ablation output directory already exists: $ABLATION_OUT_DIR" >&2
...
run_mode() {
  ...
  echo "[*] Metadata online filter ablation mode=$mode fanogan_enabled=$fanogan_enabled"
  ...
  OUT_DIR="$mode_out_dir" "$PYTHON_BIN" "$ROOT/scripts/summarize_alfresco_afl_metadata_online_filter_smoke.py"
}
...
echo "[OK] Metadata online filter ablation runs completed under $ABLATION_OUT_DIR"
