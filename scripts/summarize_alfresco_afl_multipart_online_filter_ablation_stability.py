#!/usr/bin/env python3
"""Summarize short stability runs for four multipart_upload online filter modes."""
...
BASE_DIR = Path(
    os.environ.get(
        "STABILITY_OUT_DIR",
        ROOT / "out" / "alfresco_afl_multipart_online_filter_ablation_stability",
    )
)
...
SUMMARY_SOURCE = "afl_fuzz_multipart_online_filter_ablation_stability"
EXECUTION_SCOPE = "alfresco_multipart_upload_mock_afl_online_filter_four_mode_short_stability"
METRIC_SEMANTICS = (
    "Short stability experiment over four multipart_upload SE-fAnoGAN-ES-style online filter modes "
    "in representative AFL++ mutation-chain; mock target only, not full SE-fAnoGAN-ES and not real "
    "Alfresco service."
)
...
def collect_run(mode: str, run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "filename_detected_count": int_value(summary.get("filename_detected_count")),
        "content_type_detected_count": int_value(summary.get("content_type_detected_count")),
...
def fanogan_observation(
    runs_by_mode: dict[str, list[dict[str, Any]]],
    scoring_error_total: int,
) -> str:
    ...
    rule_ae_execs = sum(int(row["execs_done"]) for row in runs_by_mode["rule_ae"])
    fanogan_execs = sum(int(row["execs_done"]) for row in runs_by_mode["rule_fanogan"])
    ae_fanogan_execs = sum(int(row["execs_done"]) for row in runs_by_mode["rule_ae_fanogan"])
    if scoring_error_total or fanogan_errors:
    ...
    if rule_ae_execs and (fanogan_execs < rule_ae_execs * 0.5 or ae_fanogan_execs < rule_ae_execs * 0.5):
        return "fanogan_online_path_stable_without_scoring_error_but_inference_cost_is_higher_keep_ae_v1_primary"
    return "fanogan_online_path_stable_without_scoring_error_continue_evaluation"
...
def main() -> int:
    ...
    total_tmout = sum(int(row["total_tmout"]) for row in rows)
    ...
    summary_row = {
        ...
        "rule_only_total_sent_to_target": sum(int(row["sent_to_target"]) for row in runs_by_mode["rule_only"]),
        "rule_ae_total_sent_to_target": sum(int(row["sent_to_target"]) for row in runs_by_mode["rule_ae"]),
        "rule_fanogan_total_sent_to_target": sum(int(row["sent_to_target"]) for row in runs_by_mode["rule_fanogan"]),
        "rule_ae_fanogan_total_sent_to_target": sum(int(row["sent_to_target"]) for row in runs_by_mode["rule_ae_fanogan"]),
        "ae_primary_recommendation": "keep_ae_v1_as_primary"
        if overall_stability_score == 1.0
        else "investigate_online_filter_stability",
        "fanogan_online_stability_observation": fanogan_observation(runs_by_mode, scoring_error_total),
    ...
    report = {
        ...
        "summary": summary_row,
        "boundary": [
            "multipart_upload online filter four-mode short stability only",
