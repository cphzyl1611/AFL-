...
STABILITY_OUT_DIR="${STABILITY_OUT_DIR:-$ROOT/out/alfresco_afl_online_filter_ablation_stability}"
RUNS="${RUNS:-3}"
DUR="${DUR:-20}"
...
run_mode_once() {
  ...
  echo "[*] Online filter ablation stability mode=$mode run=$run_id fanogan_enabled=$fanogan_enabled"
  OUT_DIR="$run_dir" \
  ...
  OUT_DIR="$run_dir" "$PYTHON_BIN" "$ROOT/scripts/summarize_alfresco_afl_online_filter_smoke.py"
}
...
echo "[OK] Online filter ablation stability runs completed under $STABILITY_OUT_DIR"
