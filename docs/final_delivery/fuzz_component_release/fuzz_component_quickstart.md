...
- O2OA HTTP/REST JSON 数据接口场景种子集
- fuzz_test_runner 外层控制器
- 20/60 秒两段式真实执行脚本

口径边界：

- O2OA 主线当前以 O2OA 数据接口场景作为阶段验收口径，证据集中在 `cms_doc_list`，并有 `calendar_filter`、`review_count`、`hotpic_list`、`person_detail` 等接口配置支撑。不要把当前证据表述为已经完整覆盖“电子公文创建 / 保存 / 更新”的全部业务流程。
- Flowable 仅作为第二平台最小校准验证，用于证明“新平台需要专用小数据集校准”这一流程成立；当前 Flowable 验证不是完整 AFL++ 变异模糊测试主链。
...
注意：执行前必须人工核验 `model_stage/models/sefanogan_ae_model.pt` 与 `model_stage/models/sefanogan_ae_meta.json` 确实对应 O2OA 默认 AE@1.0。若模型文件仍与 Flowable v3 文件一致，不得将该命令作为最终交付复现依据。
...
- v1 已被 v2@6.3 替代；最终使用时不要优先引用 v1@12.4
...
当前 Flowable 链路用于第二平台最小校准验证：以已构造的混合数据集进行模型打分、pass/reject 统计和 Flowable REST POST 验证。该链路不等同于 O2OA 主线中的完整 AFL++ 变异执行闭环，不能在汇报中表述为“Flowable 平台完整 fuzz 主链已迁移完成”。
