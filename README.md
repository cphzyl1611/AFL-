...
- 代表性 AFL++ mutation-chain smoke、短时稳定性实验、online filter 原型、metadata_update online filter smoke、四模式消融与四模式短时稳定性实验；
...
- 代表性 AFL++ mutation-chain smoke：已新增 Alfresco `content_update` 本地 mock target 短时 AFL++ smoke，并完成 3 轮短时稳定性实验；已新增 SE-fAnoGAN-ES-style online filter prototype、`metadata_update` / application/json online filter smoke、`rule_only`/`rule_ae`/`rule_fanogan`/`rule_ae_fanogan` 四模式消融和四模式 3 轮短时稳定性实验，默认主机制仍为 AE v1，fAnoGAN candidate 在线路径已验证但未替代 AE v1；这些链路不访问真实 Alfresco 服务，不代表所有场景完整 AFL++ mutation-chain；
...
| Alfresco metadata_update online filter smoke | 已完成短时真实 AFL++ smoke | 使用 application/json 本地 mock target，默认 `rule_ae`，AE v1 仍为主机制；evidence 位于 `out/alfresco_afl_metadata_online_filter_smoke_latest/summary.csv`、`eval_report.json` 和 `fuzzer_stats`。 |
