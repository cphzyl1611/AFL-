...
- 代表性 AFL++ mutation-chain smoke 与短时稳定性实验；
...
- 代表性 AFL++ mutation-chain smoke：已新增 Alfresco `content_update` 本地 mock target 短时 AFL++ smoke，并完成 3 轮短时稳定性实验，证明至少一条真实 AFL++ 变异执行链路具备短时稳定性；该 smoke 不访问真实 Alfresco 服务，不代表所有场景完整 AFL++ mutation-chain；
