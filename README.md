...
- 代表性 AFL++ mutation-chain smoke、短时稳定性实验、online filter 原型与四模式消融；
...
- 代表性 AFL++ mutation-chain smoke：已新增 Alfresco `content_update` 本地 mock target 短时 AFL++ smoke，并完成 3 轮短时稳定性实验；已新增 SE-fAnoGAN-ES-style online filter prototype 和 `rule_only`/`rule_ae`/`rule_fanogan`/`rule_ae_fanogan` 四模式消融，默认主机制仍为 AE v1，fAnoGAN candidate 在线路径已验证但未替代 AE v1；该链路不访问真实 Alfresco 服务，不代表所有场景完整 AFL++ mutation-chain；
...
| SE-fAnoGAN-ES-style online filter prototype | 已完成代表性 AFL++ online filter smoke 与四模式消融 | 默认 `rule_ae`，AE v1 在线判定后再执行本地 mock target；四模式消融验证了 fAnoGAN candidate 在线路径可运行且无 scoring error，但短时样本量受 torch scorer 成本限制，AE v1 仍为主机制。 |
