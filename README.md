...
- Alfresco fAnoGAN v1 candidate：已完成离线 GAN-style 候选评分、扩展样本二次评估与 AE v1 对比，当前 recommendation 为继续以 AE v1 作为主二阶段判定；
...
| Alfresco fAnoGAN v1 candidate | 已完成离线候选对比和扩展样本二次评估 | 当前为 `gan_style_statistical_candidate`，用于和 AE v1 对比；扩展样本下 `rule_ae=false_accept=0,false_reject=4`，`rule_fanogan=false_accept=0,false_reject=5`，candidate 未优于 AE v1。 |
