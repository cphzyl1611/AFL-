...
- Alfresco fAnoGAN v1 candidate：已完成 fallback 与 `.venv` torch fAnoGAN-style 候选评分、扩展样本二次评估、劣势诊断和 AE v1 对比；当前 recommendation 仍为继续以 AE v1 作为主二阶段判定；
...
| Alfresco fAnoGAN v1 candidate | 已完成 fallback 与 torch 候选对比、扩展样本二次评估和诊断复评 | 当前 `.venv` 复评为 `torch_fanogan_style_candidate`，用于和 AE v1 对比；扩展样本下 `rule_ae=false_accept=0,false_reject=4`，`rule_fanogan=false_accept=0,false_reject=5`，candidate 未优于 AE v1，recommendation 仍为 `keep_ae_v1_as_primary`。 |
