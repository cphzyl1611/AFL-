...
class AlfrescoMultipartOnlineFilterAblationStabilityTest(unittest.TestCase):
    def test_run_script_exists(self) -> None:
        path = REPO_ROOT / "scripts/run_alfresco_afl_multipart_online_filter_ablation_stability.sh"
        self.assertTrue(path.is_file())

    def test_summarizer_compiles(self) -> None:
        py_compile.compile(
            str(REPO_ROOT / "scripts/summarize_alfresco_afl_multipart_online_filter_ablation_stability.py"),
            doraise=True,
        )

    def test_optional_stability_summary_header(self) -> None:
        summary_path = REPO_ROOT / "out/alfresco_afl_multipart_online_filter_ablation_stability/stability_summary.csv"
        if not summary_path.exists():
            self.skipTest("optional multipart online filter ablation stability summary is not present")
