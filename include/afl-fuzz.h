...
struct queue_entry {
  ...
  /* ==== SS (Security State) scheduling fields ==== */
  u64    ss_cov_cnt;       /* how many times this seed produced new ss/coverage */
  u64    ss_selected_cnt;  /* how many times this seed was selected */
  double ss_prob;          /* current probability weight */
#ifdef INTROSPECTION
  u32 bitsmap_size;
#endif
...
typedef enum {
  NV_TARGET_UNKNOWN = 0,
  NV_TARGET_HTTP_API = 1,
  NV_TARGET_BINARY = 2
} nv_target_type_t;

typedef struct {

  nv_target_type_t target_type;
  u8 *target_endpoint;
  u8 *seed_source;
  u8 *seed_location;
  u32 mutation_scope;
  ...
  /* --- NV 2.4 validity --- */
  u8   enable_validity;         /* 0/1 */
  double validity_threshold;    /* MVP: used later */
  u8 *validity_endpoint;        /* unix://... or tcp://... (MVP optional) */

} nv_task_cfg_t;
typedef enum {
  NV_ARM_FIELD_VALUE = 0,
  NV_ARM_BOUNDARY    = 1,
  NV_ARM_STRUCTURE   = 2,
  NV_ARM_MAX
} nv_arm_id_t;
...
typedef struct afl_state {
  u64 nv_valid_cnt;
  u64 nv_invalid_cnt;
  u64 nv_invalid_parse_cnt;
  u64 nv_invalid_rule_cnt;
  u64 nv_invalid_rpc_fail_cnt;
  /* --- NV 2.5 endogenous metrics --- */
  u64 nv_total_valid_exec;
  u64 nv_err_exec;
  u64 nv_rec_total;
  u64 nv_rec_success;
  u64 nv_rec_time_sum_ms;
  nv_task_cfg_t nv_task;
  u8 *nv_task_path; /*新增*/
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u32  nv_cov_cap;
  u32  nv_cov_used;
  u64 nv_http_status_ok;
  u64 nv_http_status_fail;
  u64 nv_ncov_hit;
  u64 nv_last_status_seq;
  u64 nv_err_cnt;
  u64 nv_rec_cnt;
  u64 nv_rec_ms_sum;
  u64 nv_status_cnt;
  ...
  char **nv_allow_pairs;   /* array of "METHOD PATH" strings */

  /* optional: for debug */
  char *nv_target_cfg_path;
  u64 nv_invalid_allow_cnt;
  u64 nv_invalid_docid_cnt;
  u64 nv_invalid_body_cnt;
  u64 nv_invalid_score_cnt;
} afl_state_t;
