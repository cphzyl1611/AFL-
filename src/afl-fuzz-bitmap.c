...
#include <stdlib.h>
#include "asanfuzz.h"
/* ===== SS debug counters for verifying 2.2 feedback path ===== */
u64 ss_new_bits_any  = 0;
u64 ss_new_bits_kept = 0;
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  save_to_queue:
    ...
    if (new_bits) ss_new_bits_any++;
    if (keeping && new_bits) ss_new_bits_kept++;
    /* reward the seed that produced this interesting input */
