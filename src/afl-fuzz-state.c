...
void afl_state_deinit(afl_state_t *afl) {
  ...
  if (afl->nv_task.target_endpoint) {

    ck_free(afl->nv_task.target_endpoint);
    afl->nv_task.target_endpoint = NULL;

  }

  if (afl->nv_task.seed_source) {

    ck_free(afl->nv_task.seed_source);
    afl->nv_task.seed_source = NULL;

  }

  if (afl->nv_task.seed_location) {

    ck_free(afl->nv_task.seed_location);
    afl->nv_task.seed_location = NULL;

  }

  if (afl->nv_task.validity_endpoint) {
    ck_free(afl->nv_task.validity_endpoint);
    afl->nv_task.validity_endpoint = NULL;
  }
