...
static u8 *nv_read_entire_file(const char *path, size_t *out_len) {
  ...
  if (!f) { FATAL("Cannot open nv_task file: %s", path); }

  if (fseek(f, 0, SEEK_END) != 0) { fclose(f); FATAL("fseek failed: %s", path); }
  sz = ftell(f);
  if (sz < 0) { fclose(f); FATAL("ftell failed: %s", path); }
  rewind(f);
  ...
  if (rd != (size_t)sz) { FATAL("Short read from %s", path); }
  buf[sz] = 0;
...
static u32 nv_parse_scope(cJSON *scope) {
  ...
  const u32 DEFAULT_SCOPE = 0x7; /* 1|2|4 */

  if (!scope) return DEFAULT_SCOPE;

  /* numeric bitmask, e.g. 7 */
  if (cJSON_IsNumber(scope)) {

    u32 v = (u32)scope->valuedouble;
    v &= 0x7;

    if (!v) return DEFAULT_SCOPE; /* avoid zero disabling everything */
    return v;
...
static nv_target_type_t nv_parse_target_type(cJSON *tt) {

  if (!tt) return NV_TARGET_UNKNOWN;

  if (cJSON_IsNumber(tt)) {

    int v = tt->valueint;
    if (v == 1) return NV_TARGET_HTTP_API;
    if (v == 2) return NV_TARGET_BINARY;
    return NV_TARGET_UNKNOWN;

  }

  if (cJSON_IsString(tt) && tt->valuestring) {

    if (!strcmp(tt->valuestring, "http_api") ||
        !strcmp(tt->valuestring, "http") ||
        !strcmp(tt->valuestring, "rest")) {

      return NV_TARGET_HTTP_API;

    }

    if (!strcmp(tt->valuestring, "binary")) return NV_TARGET_BINARY;
...
/* return 1 if "METHOD PATH" exists in allow list */
int nv_is_allowed_pair(afl_state_t *afl, const char *method, const char *path) {

  if (!afl || !method || !path) return 0;
  if (!afl->nv_tcfg_loaded) return 1; /* if no cfg, do not block */

  char key[1024];
  snprintf(key, sizeof(key), "%s %s", method, path);

  for (u32 i = 0; i < afl->nv_allow_cnt; i++) {

    if (!afl->nv_allow_pairs[i]) continue;

    const char *rule = afl->nv_allow_pairs[i];
    size_t n = strlen(rule);

    /* 1) exact match */
    if (strcmp(rule, key) == 0) return 1;

    /* 2) prefix match: rule ends with '*' */
    if (n > 0 && rule[n - 1] == '*') {
      if (strncmp(rule, key, n - 1) == 0) return 1;
    }
...
void nv_load_target_cfg(afl_state_t *afl) {

  if (!afl || afl->nv_tcfg_loaded) return;

  const char *p = getenv("NV_TARGET_CONFIG");
  if (!p || !*p) {
    afl->nv_tcfg_loaded = 0;
    return;
  }

  afl->nv_target_cfg_path = ck_strdup((u8 *)p);

  FILE *fp = fopen(p, "rb");
  if (!fp) { afl->nv_tcfg_loaded = 0; return; }

  fseek(fp, 0, SEEK_END);
  long sz = ftell(fp);
  fseek(fp, 0, SEEK_SET);
  if (sz <= 0 || sz > 1024 * 1024) { fclose(fp); afl->nv_tcfg_loaded = 0; return; }

  char *buf = ck_alloc(sz + 1);
  if (fread(buf, 1, sz, fp) != (size_t)sz) {
    ck_free(buf); fclose(fp); afl->nv_tcfg_loaded = 0; return;
  }
  buf[sz] = 0;
  fclose(fp);
  ...
  ck_free(buf);
  if (!root) { afl->nv_tcfg_loaded = 0; return; }

  cJSON *eps = cJSON_GetObjectItemCaseSensitive(root, "endpoints");
  if (!cJSON_IsArray(eps)) { cJSON_Delete(root); afl->nv_tcfg_loaded = 0; return; }
  ...
  cJSON_ArrayForEach(it, eps) {
    ...
    cJSON *m = cJSON_GetObjectItemCaseSensitive(it, "method");
    cJSON *pa = cJSON_GetObjectItemCaseSensitive(it, "path");
    if (!cJSON_IsString(m) || !m->valuestring) continue;
    if (!cJSON_IsString(pa) || !pa->valuestring) continue;
    cnt++;
  }

  afl->nv_allow_cnt = cnt;
  afl->nv_allow_pairs = ck_alloc(cnt * sizeof(char *));
  memset(afl->nv_allow_pairs, 0, cnt * sizeof(char *));
  ...
  cJSON_ArrayForEach(it, eps) {
    ...
    const char *ps = pa->valuestring;

    char key[1024];
    snprintf(key, sizeof(key), "%s %s", ms, ps);
    afl->nv_allow_pairs[idx++] = ck_strdup((u8 *)key);
    if (idx >= cnt) break;
  }

  afl->nv_tcfg_loaded = 1;
  cJSON_Delete(root);

}

static void nv_load_task_json(afl_state_t *afl, const char *path) {

  size_t len = 0;
  u8    *data = nv_read_entire_file(path, &len);

  cJSON *root = cJSON_Parse((char *)data);
  if (!root) {

    ck_free(data);
    FATAL("nv_task JSON parse failed: %s", path);

  }

  cJSON *tt   = cJSON_GetObjectItemCaseSensitive(root, "target_type");
  cJSON *ep   = cJSON_GetObjectItemCaseSensitive(root, "target_endpoint");
  cJSON *ss   = cJSON_GetObjectItemCaseSensitive(root, "seed_source");
  cJSON *sl   = cJSON_GetObjectItemCaseSensitive(root, "seed_location");
  cJSON *ms   = cJSON_GetObjectItemCaseSensitive(root, "mutation_scope");
  cJSON *mtc  = cJSON_GetObjectItemCaseSensitive(root, "max_test_cases");
  cJSON *tb   = cJSON_GetObjectItemCaseSensitive(root, "time_budget");
  cJSON *ev  = cJSON_GetObjectItemCaseSensitive(root, "enable_validity");
  cJSON *vth = cJSON_GetObjectItemCaseSensitive(root, "validity_threshold");
  cJSON *vep = cJSON_GetObjectItemCaseSensitive(root, "validity_endpoint");
  

  afl->nv_task.target_type = nv_parse_target_type(tt);

  if (ep && cJSON_IsString(ep) && ep->valuestring) afl->nv_task.target_endpoint = ck_strdup(ep->valuestring);
  if (ss && cJSON_IsString(ss) && ss->valuestring) afl->nv_task.seed_source     = ck_strdup(ss->valuestring);
  if (sl && cJSON_IsString(sl) && sl->valuestring) afl->nv_task.seed_location   = ck_strdup(sl->valuestring);
  

  afl->nv_task.mutation_scope = nv_parse_scope(ms);
  ...
  if (tb  && cJSON_IsNumber(tb))  afl->nv_task.time_budget_sec = (u64)tb->valuedouble;
    /* --- NV 2.4 validity --- */
  afl->nv_task.enable_validity = 0;      /* default off */
  afl->nv_task.validity_threshold = 0.0; /* default */
  afl->nv_task.validity_endpoint = NULL; /* default */

  /* enable_validity: accept bool(true/false) OR number(0/1) */
  if (ev) {
    if (cJSON_IsBool(ev)) {
      afl->nv_task.enable_validity = cJSON_IsTrue(ev) ? 1 : 0;
    } else if (cJSON_IsNumber(ev)) {
      afl->nv_task.enable_validity = (ev->valuedouble != 0) ? 1 : 0;
    }
  }

  /* validity_threshold: number */
  if (vth && cJSON_IsNumber(vth)) {
    afl->nv_task.validity_threshold = vth->valuedouble;
  }

  /* validity_endpoint: string */
  if (vep && cJSON_IsString(vep) && vep->valuestring) {
    afl->nv_task.validity_endpoint = ck_strdup(vep->valuestring);
  }

  if (afl->nv_task.target_type == NV_TARGET_UNKNOWN) {

    WARNF("nv_task: target_type missing/unknown, defaulting to UNKNOWN");

  }

  /* --- record nv_task_path for stats / reproducibility --- */
  if (path && *path) {
    if (afl->nv_task_path) {
      ck_free(afl->nv_task_path);
      afl->nv_task_path = NULL;
    }
    afl->nv_task_path = ck_strdup(path);
  }

  ACTF("NV_TASK loaded: type=%d endpoint=%s seeds=%s/%s scope=0x%x max=%llu time=%llus "
      "enable_validity=%u vth=%.3f vep=%s",
      ...
      afl->nv_task.validity_threshold,
      afl->nv_task.validity_endpoint ? (char*)afl->nv_task.validity_endpoint : "(null)");
  
  nv_load_target_cfg(afl);
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--nv_task")) argv[i] = "-J";
  }
  ...
  while ((opt = getopt(
    ...
    switch (opt) {
      ...
      case 'J':
        if (afl->nv_task_path)
            FATAL("Multiple -J/--nv_task options not supported");
        if (!optarg)
            FATAL("Missing path for -J/--nv_task");

        afl->nv_task_path = ck_strdup(optarg);
        nv_load_task_json(afl, optarg);
        break;
      case 'w': {
  ...
  if (unlikely(fast_resume)) {
    ...
    if (likely(!afl->afl_env.afl_no_startup_calibration)) {
       char *nv_task_path = getenv("NV_TASK_PATH");
      if (nv_task_path && *nv_task_path) { nv_load_task_json(afl, nv_task_path); }
  ...
  while (likely(!afl->stop_soon)) {
    /* --- NV 2.6: stop by time budget (sec) --- */
    if (afl->nv_task.time_budget_sec) {

      u64 elapsed_ms = get_cur_time() - afl->start_time;
      if (unlikely(elapsed_ms >= afl->nv_task.time_budget_sec * 1000ULL)) {

        afl->stop_soon = 2;  /* programmatic graceful stop */
        goto stop_fuzzing;
...
stop_fuzzing:
  ...
  /* --- NV (2.1验收)：退出时强制写一次 fuzzer_stats --- */
  {
    /* 退出时可能处于 non-instrumented mode(-n)，但我们仍要落盘 stats 以便验收 */
    u8 old_noninst = afl->non_instrumented_mode;
    afl->non_instrumented_mode = 0;

    u32 t_bytes = count_non_255_bytes(afl, afl->virgin_bits);
    double t_byte_ratio = 0.0;
    if (afl->fsrv.real_map_size) {
      t_byte_ratio = ((double)t_bytes * 100.0) / afl->fsrv.real_map_size;
    }

    double stab_ratio = 100.0;
    if (t_bytes && afl->var_byte_count) {
      stab_ratio = 100.0 - (((double)afl->var_byte_count * 100.0) / t_bytes);
    }

    /* stats_avg_exec 已在 stats 逻辑中维护；没有也没关系，0 也能写 */
    write_stats_file(afl, t_bytes, t_byte_ratio, stab_ratio, afl->stats_avg_exec);

    afl->non_instrumented_mode = old_noninst;
  }
